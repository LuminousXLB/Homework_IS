...
class LZEncoderByte : public LZEncoderBase<uint8_t, 256> {
public:
    LZEncoderByte(string infile, string outfile)
    {
        io.open(new iByteUnit(), infile, new oBitByte(), outfile);
    }
};

class LZDecoderByte : public LZDecoderBase<uint8_t, 256> {
...
public:
    LZDecoderByte(string infile, string outfile)
    {
        io.open(new iBitByte(), infile, new oByteUnit(), outfile);
    }
};
